...
class IndicoTimetableParser(HTMLParser):
    ...
    def handle_endtag(self, tag: str) -> None:
        ...
        if self.item is not None and tag == "li":
            cleaned = {k: " ".join(v.split()) for k, v in self.item.items()}
...
def detect_source_timezone(*pages: str, override: str | None) -> ZoneInfo:
    if override:
...
def item_datetimes(
    ...
    start_source = datetime.fromisoformat(f"{item.date}T{item.time}:00").replace(tzinfo=source_tz)
...
def write_ics(
    ...
    source_tz_name = getattr(source_tz, "key", str(source_tz))
    lines = [
        "BEGIN:VCALENDAR",
        "VERSION:2.0",
    ...
    for item in items:
        ...
        lines.extend(
            [
                ...
                f"  set theLocation to {apple_quote(location)}",
...
def print_summary(items: list[TimetableItem], source_tz: ZoneInfo, local_tz: ZoneInfo) -> None:
    ...
    print(f"source_timezone={getattr(source_tz, 'key', source_tz)}")
    print(f"local_timezone={getattr(local_tz, 'key', local_tz)}")
    if not items:
        return
...
def main() -> int:
    ...
    try:
        ...
        overview_page = ""
    source_tz = detect_source_timezone(page, overview_page, override=args.source_timezone)
